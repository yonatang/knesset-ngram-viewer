...
import edu.berkeley.nlp.util.Logger; //import edu.berkeley.nlp.util.optionparser.GlobalOptionParser;
...
public class StochasticObjectiveOptimizer<I> {
	...
	class GradMapper implements SimpleMapper<I> {
		...
		public void map(I elem) {
			...
			if (regularizer != null) {
				val += regularizer.update(localWeights, localGrad, 1.0 / items
						.size());
			}
			synchronized (weightLock) {
