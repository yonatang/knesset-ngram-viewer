...
public class Table {
	...
	public static abstract class Populator {
		...
		public final void set(int row, int column, String text) {
			...
			table.maxHeights.setCount(row, Math.max(table.maxHeights
					.getCount(row), lines.length));
			for (String line : lines)
				table.maxWidths.setCount(column, Math.max(table.maxWidths
						.getCount(column), line.length()));
		...
		for (int row = 0; row < nRows; row++) {
			for (int rowLine = 0; rowLine < maxHeights.getCount(row); rowLine++) {
				...
				for (int col = 0; col < nColumns; col++) {
					...
					builder
							.append(pad(text, (int) maxWidths.getCount(col),
									" "));
					if (printBorders)
						builder.append("|");
				}
