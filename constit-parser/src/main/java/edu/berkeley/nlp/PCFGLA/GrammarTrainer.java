...
public class GrammarTrainer {
	...
	public static class Options {
		...
		@Option(name = "-lex2", usage = "use lex2 (YG)")
		public boolean lex2 = false;
	...
	public static void main(String[] args) {
		...
		if (splitGrammarFile == null) {
            ...
            : opts.lex2 ?
					  new SophisticatedLexicon5(numSubStatesArray, SophisticatedLexicon.DEFAULT_SMOOTHING_CUTOFF, smoothParams, new NoSmoothing(), filter)
		...
		for (int splitIndex = startSplit; splitIndex < numSplitTimes * 3; splitIndex++) {
			...
			if (splitIndex % 3 == 2) {// (splitIndex==numSplitTimes*2){
				...
				if (opts.featurizedLexicon) {
                  ...
                  : opts.lex2 ?
							  new SophisticatedLexicon5( newNumSubStatesArray, SophisticatedLexicon.DEFAULT_SMOOTHING_CUTOFF, maxLexicon.getSmoothingParams(), maxLexicon .getSmoother(), maxLexicon .getPruningThreshold())
				...
				// FeaturizedLexicon(numSubStatesArray,feat,trainStateSetTrees);
					...
					lexicon = (opts.simpleLexicon) ? new SimpleLexicon(
							grammar.numSubStates, -1, smoothParams, lexicon
									.getSmoother(), filter, trainStateSetTrees)
							: opts.lex2 ?
                      new SophisticatedLexicon5( grammar.numSubStates, SophisticatedLexicon.DEFAULT_SMOOTHING_CUTOFF, lexicon.getSmoothingParams(), lexicon .getSmoother(), lexicon .getPruningThreshold())
