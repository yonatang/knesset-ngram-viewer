...
public class GrammarTester implements Callable {
	...
	public static void main(String[] args) {
		...
		EnglishPennTreebankParseEvaluator.LabeledConstituentEval<String> eval = new EnglishPennTreebankParseEvaluator.LabeledConstituentEval<String>(
				new HashSet<String>(Arrays.asList(new String[] { "ROOT",
						"PSEUDO" })), new HashSet<String>(Arrays
						.asList(new String[] { "''", "``", ".", ":", "," })));
		...
		if (!opts.testAll) {
			...
			for (Tree<String> testTree : testTrees) {
				...
				if (opts.evaluateConstraints && cons != null) {
					...
					numPruned = countPrunedNodes(allowedStates, Numberer
							.getGlobalNumberer("tags"), false, 0, testTree
							.getYield().size());
		...
		for (int state = 0; state < cons.length; ++state) {
			...
			if (!isPreTerminal)
				unannotatedLabel = TreeAnnotations.unAnnotateTree(
						new Tree<String>(asString, Collections
								.singletonList(new Tree<String>("FakeLabel"))))
						.getLabel();
	...
	private static int countPrunedNodes(Tree<String> tree,
		...
		int total = 0;
		if (!isAllowed(tree.getLabel(), tagNumberer, cons[from][to], tree
				.isPreTerminal())) {
			total += 1;
		}
	...
	public Pair<EnglishPennTreebankParseEvaluator.LabeledConstituentEval<String>, String> call()
			throws Exception {
		EnglishPennTreebankParseEvaluator.LabeledConstituentEval<String> eval = new EnglishPennTreebankParseEvaluator.LabeledConstituentEval<String>(
				Collections.singleton("ROOT"), new HashSet<String>(Arrays
						.asList(new String[] { "''", "``", ".", ":", "," })));
