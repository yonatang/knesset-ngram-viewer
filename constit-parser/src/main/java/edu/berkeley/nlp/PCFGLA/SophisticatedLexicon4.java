 ...
 */
public class SophisticatedLexicon4 extends SophisticatedLexicon {
   ...
   //YG this is new
	...
	public SophisticatedLexicon4(short[] numSubStates, int smoothingCutoff,
	...
	public SophisticatedLexicon splitAllStates(int[] counts, boolean moreSubstatesThanCounts, int mode) { 
		...
		SophisticatedLexicon4 lexicon = new SophisticatedLexicon4(
	...
	public double[] score(String origWord, short tag, int loc, boolean noSmoothing,
      ...
      for (int i=0; i<sigs.length; i++) {
		...
		for (int substate = 0; substate < numSubStates[tag]; substate++) {
            ...
            for (int i=0;i<sigs.length;i++) {
               String sig = sigs[i];
               pb_W_T += s_g_w[i]*sigScore(word, sig, tag, substate);
            }
			}
   ...
   //@@YG I assume that if randomness >= 0, then this is the initialization.
	public void trainTree(Tree<StateSet> trainTree, double randomness,
		...
		if (oldLexicon != null) {
			...
			oldSigCounter = ((SophisticatedLexicon4)oldLexicon).getSigCounter();
		}
   ...
   //@@YG this is the original. TODO add unseen/rare words info?
	...
	public SophisticatedLexicon4 projectLexicon(double[] condProbs,
		...
		SophisticatedLexicon4 newLexicon = new SophisticatedLexicon4(
	...
	public SophisticatedLexicon4 copyLexicon() {
	...
	@SuppressWarnings("unchecked")
	public SophisticatedLexicon4 remapStates(Numberer thisNumberer,
