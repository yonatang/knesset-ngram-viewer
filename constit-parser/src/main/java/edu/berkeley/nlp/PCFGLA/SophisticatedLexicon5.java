 ...
 */
public class SophisticatedLexicon5 extends SophisticatedLexicon {
   ...
   //YG this is new
	...
	public String toString() {
		...
		StringBuilder sb = new StringBuilder();
         ...
         for (String sig : this.sigCounter.keySet()) {
            for (int subs=0; subs<this.numSubStates[tag]; subs++) {
               double score = this.sigScore("",sig, (short)tag, subs);
               if (score>0) {
                  sb.append("SIG:" + sig + "," + tagState + "-" + subs + "=" + score + "\n");
               }
            }
         }
		}
	...
	public SophisticatedLexicon5(short[] numSubStates, int smoothingCutoff,
	...
	public SophisticatedLexicon splitAllStates(int[] counts, boolean moreSubstatesThanCounts, int mode) { 
		...
		SophisticatedLexicon5 lexicon = new SophisticatedLexicon5(
    ...
    * @return A double valued score, usually P(word|tag)
	...
	public double[] score(String origWord, short tag, int loc, boolean noSmoothing,
      ...
      double[] s_g_w = new double[sigs.length];
      for (int i=0; i<sigs.length; i++) {
         s_g_w[i]=Double.parseDouble(sigs[i].split("/")[1]);
         sigs[i]=sigs[i].split("/")[0];
      }
		...
		for (int substate = 0; substate < numSubStates[tag]; substate++) {
			...
			if (!isSignature && (seen || noSmoothing)) {
               ...
               double pb_T_W_unk = 0; //@@@YG here now!
               // pb_T_W_unk = calculate_pb_T_W_unk(tag, substate, word, s_g_w, sigs);
               for (int i=0; i<sigs.length; i++) {
                  pb_T_W_unk += sigScore(word, sigs[i], tag, substate)*s_g_w[i];
               }
               pb_T_W = (pb_T_W_unk) + ((c_W)*pb_T_W) / (1+c_W);
               ...
               for (int i=0;i<sigs.length;i++) {
                  String sig = sigs[i];
                  pb_W_T2 += s_g_w[i]*sigScore(word, sig, tag, substate);
               }
   ...
   //@@YG I assume that if randomness >= 0, then this is the initialization.
	public void trainTree(Tree<StateSet> trainTree, double randomness,
		...
		if (oldLexicon != null) {
			...
			oldSigCounter = ((SophisticatedLexicon5)oldLexicon).getSigCounter();
		}
         ...
         for (String sig : sigs) {
			...
			for (short substate = 0; substate < nSubStates; substate++) {
               ...
               for (int i=0;i<sigs.length;i++) {
                  ...
                  sigCounter.incrementCount(sig, weight*s_g_w[i]);

                  double[] substateCounter2 = unseenWordToTagCounters[tag].get(sig);
                  substateCounter2[substate] += weight*s_g_w[i]; 

                  unseenTagCounter[tag][substate] += weight*s_g_w[i];
   ...
   //@@YG this is the original. TODO add unseen/rare words info?
	...
	public SophisticatedLexicon5 projectLexicon(double[] condProbs,
		...
		SophisticatedLexicon5 newLexicon = new SophisticatedLexicon5(
	...
	public SophisticatedLexicon5 copyLexicon() {
	...
	@SuppressWarnings("unchecked")
	public SophisticatedLexicon5 remapStates(Numberer thisNumberer,
