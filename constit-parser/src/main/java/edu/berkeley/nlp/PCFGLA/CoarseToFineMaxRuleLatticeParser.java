...
public class CoarseToFineMaxRuleLatticeParser extends CoarseToFineMaxRuleParser {
	...
	public CoarseToFineMaxRuleLatticeParser(Grammar gr, Lexicon lex,
		...
		grammar = gr;
		lexicon = (ExtLexicon)lex; // THIS IS UGLY, should change in sig, but I no want to change all the code..
	...
	public void initCascade(CoarseToFineMaxRuleLatticeParser otherParser) {
	...
	void doConstrainedOutsideScores(Grammar grammar, boolean viterbi,
		...
		for (int diff = length; diff >= 1; diff--) {
			for (int start = 0; start + diff <= length; start++) {
				int end = start + diff;
				// do unaries
				double[][] scoresAfterUnaries = new double[numStates][];    // @WHY
																			// two
																			// entries
																			// and
																			// not
	...
	// @@ YG: new func. copied straight from oldblatt. changes?
	void initializeChartFromLattice(LatticeInput lat, Lexicon lex,
			boolean noSubstates, boolean noSmoothing, boolean scale) {
		// @@YGYG the "real" ones fill the lattice this way (all bad tags get
		// -Infinity)
		if (noSubstates) {
			for (int start = 0; start < lat.length(); start++) {
				int end = start + 1;
				...
				Arrays.fill(viScore[start][end], Double.NEGATIVE_INFINITY);
			}
		}
		for (WordData wd : lat.spansData()) {
			...
			// { allowedStates[start][end][tag] = true; } // @@YGYG
			if (!noSubstates && !allowedStates[start][end][tag])
				continue; // @@YGYG??
			//System.out.println("WDinit:" + start + " " + end + " " + tag);
			/* if (grammarTags[tag]) continue; */assert (!grammarTags[tag]);
			if (end < narrowRExtent[start][tag])
				narrowRExtent[start][tag] = end;
			if (start > narrowLExtent[end][tag])
				narrowLExtent[end][tag] = start;
			if (end > wideRExtent[start][tag])
				wideRExtent[start][tag] = end;
			if (start < wideLExtent[end][tag])
				wideLExtent[end][tag] = start;
			double[] lexiconScores = ((ExtLexicon)lex).score(w, (short) tag, start,
					noSmoothing, false, wd.prob);
			if (scale)
			...
			for (short n = 0; n < lexiconScores.length; n++) {
				...
				if (noSubstates) {
					// System.out.println("s/e/t:" + start + "/" + end + "/" +
					// tag);
	...
	protected void createArrays(LatticeInput lat, boolean firstTime,
			int numStates, short[] numSubStatesArray, int level,
			double initVal, boolean justInit) {
		...
		if (firstTime) {
			...
			hasPosItem = new boolean[length][length+1];
		...
		for (int start = 0; start < length; start++) {
			for (int end = start + 1; end <= length; end++) {
				if (firstTime) {
					...
					for (WordData wd : lat.spansData()) {
						if (start == wd.start && end == wd.end) {
   //                  System.out.println("wd"+wd.start+","+wd.end+":"+wd.tagString);
                   try{
							allowedStates[wd.start][wd.end][wd.tagNum] = true;
                    } catch (ArrayIndexOutOfBoundsException e) {
                       System.err.println("unknown pos-tag:"+wd.tagString+ ", allowing all");
                    }
						}
					}
	...
	protected void pruneChart(double threshold, short[] numSubStatesArray,
		...
		int startDiff = (level < 0) ? 2 : 1; // @YG, for me it's always 1. when is it 2?
		for (int diff = startDiff; diff <= length; diff++) {
			for (int start = 0; start < (length - diff + 1); start++) {
				...
				for (int state = 0; state < lastState; state++) {
					//@YG this diff check is meant to "save work" for the parser
					//by telling him that if it's diff>1, than !grammarTags[state] states are not interesting.
					//but they are interesting now..
					//if (diff > 1 && !grammarTags[state])
					//	continue;
					
					// boolean allFalse = true;
					if (state == 0) { //@YG WHAT'S THIS?
	...
	public void doPreParses(LatticeInput lat, Tree<StateSet> tree,
			boolean noSmoothing) {
		boolean keepGoldAlive = (tree != null); // we are given the gold tree ->
												// make sure we don't prune it
												// away
		clearArrays();
		length = (short) lat.length();
		double score = 0;
		Grammar curGrammar = null;
		ExtLexicon curLexicon = null;
	...
	public Tree<String> getBestParse(LatticeInput lat) {
		return getBestConstrainedParse(lat, false);
	}
	...
	public Tree<String> getBestConstrainedParse(LatticeInput lat,
			boolean[][][][] allowedS) {// List<Integer>[][] pStates) {
		if (allowedS == null)
			return getBestConstrainedParse(lat, false);
		clearArrays();
		length = (short) lat.length();
		Grammar curGrammar = grammarCascade[endLevel - startLevel + 1];
		ExtLexicon curLexicon = (ExtLexicon) lexiconCascade[endLevel - startLevel + 1];
		double initVal = (viterbiParse) ? Double.NEGATIVE_INFINITY : 0;
		int level = isBaseline ? 1 : endLevel;
		allowedSubStates = allowedS;
		createArrays(lat, true, curGrammar.numStates, curGrammar.numSubStates,
				level, initVal, false);
		setConstraints(allowedS);
		return getBestConstrainedParse(lat, true);
	}
	...
	// @@YG this is really different from old.. try to understand
	public Tree<String> getBestConstrainedParse(LatticeInput lat,
			boolean noPreparse) {
		...
		double initVal = (viterbiParse) ? Double.NEGATIVE_INFINITY : 0;
		int level = isBaseline ? 1 : endLevel;
		createArrays(lat, false, curGrammar.numStates, curGrammar.numSubStates,
				level, initVal, false);
		...
		if (score != Double.NEGATIVE_INFINITY) {
			...
			if (!viterbiParse) {
				...
				doConstrainedMaxCScores(lat, curGrammar, curLexicon, false); // @@YG
																				// lst
																				// false
																				// is
																				// new
		...
		if (score != Double.NEGATIVE_INFINITY) {
			if (viterbiParse)
				bestTree = extractBestViterbiParse(0, 0, 0, length, lat);
			else
				bestTree = extractBestMaxRuleParse(0, length, lat);
		}
		// lhuang: currently can't figure out maxcScore in forest mode
	...
	void doConstrainedMaxCScores(LatticeInput lat, Grammar grammar,
			ExtLexicon lexicon, final boolean scale) {
		short[] numSubStatesArray = grammar.numSubStates;
		...
		ArrayUtil.fill(maxcScore, initVal); // @YG <-- this guy is new
		// @YG maxcScore moved to logs..

		// fill in the lexical (tag-->word) scores
		// fillTagsFromLattice(sentLat); (@@YG DO WE WANT TO MOVE THE FOLLOWIG
		// TO A FUNCTION? HOW DOES IT LOOK AT OTHER PLACES?)
		...
		// unary on top of any lexical rules. //@YG TODO do we actually allow
		// these unaries??
		for (WordData wd : lat.spansData()) {
			int start = wd.start;
			int end = wd.end;
			...
			String word = wd.form;
			assert (!grammar.isGrammarTag(tag));
			double[] lexiconScoreArray = lexicon.score(word, (short) tag, start, false, false, wd.prob);
			double lexiconScores = 0;
			for (int tp = 0; tp < nTagStates; tp++) {
				double ruleS = lexiconScoreArray[tp];
				double pOS = oScore[start][end][tag][tp];
				// if (start==3 && tag == 11) System.out.println("pOS " + start
				// + "/" + end + "/" + tag + "/" + tp + ":" +
				// oScore[start][end][tag][tp]);
				lexiconScores += (pOS * ruleS) / logNormalizer; // The
			}
			// @YG I'm not sure what the following accomplish
			...
			maxcScore[start][end][tag] = Math.log(lexiconScores)
					+ scalingFactor;
			// System.out.println("@maxcScore " + start + "/" + end + "/" +
			// Numberer.getGlobalNumberer("tags").object(tag) + ":" +
			// lexiconScores);
		}
		...
		for (int diff = 1; diff <= length; diff++) {
			...
			for (int start = 0; start < (length - diff + 1); start++) {
				...
				// Try unary rules @@YG TODO: understand unary handling: do we
				// Try unary rules
	...
	public Tree<String> extractBestMaxRuleParse(int start, int end,
			LatticeInput lat) {
		return extractBestMaxRuleParse1(start, end, 0, lat);
	}
	...
	public Tree<String> extractBestMaxRuleParse1(int start, int end, int state,
		...
		if (cState == -1) {
			return extractBestMaxRuleParse2(start, end, state, lat);
		} else {
			List<Tree<String>> child = new ArrayList<Tree<String>>();
			child.add(extractBestMaxRuleParse2(start, end, cState, lat));
	...
	public Tree<String> extractBestMaxRuleParse2(int start, int end, int state,
		...
		boolean posLevel = !grammar.isGrammarTag(state); // @YG if not grammar
		// tag, it's a POS.
		if (posLevel) {
			if (grammar.isGrammarTag(state)) {
				// @@ YG: I do not understand this if clause..
				System.out.println("I am here, but why?");
				List<Tree<String>> childs = new ArrayList<Tree<String>>();
				childs.add(new Tree<String>(lat.getWord(start, end, state)));
				String stateStr2 = (String) tagNumberer
						.object(maxcChild[start][end][state]);// +""+start+""+end;
				children.add(new Tree<String>(stateStr2, childs));
			} else
				children.add(new Tree<String>(lat.getWord(start, end, state)));
		} else {
	...
	public Tree<String> extractBestViterbiParse(int gState, int gp, int start,
        ...
        // into anything but the word below it
		if (!grammarTags[gState]) {
				List<Tree<String>> child = new ArrayList<Tree<String>>();
				child.add(new Tree<String>(sentLat.getWord(start, end, gState)));
				return new Tree<String>(goalStr, child);
		}
		// if the goal state is not a preterminal state, then find a way to
		// transform it into one
		if (end - start == 1) { // @YG TODO! there are other cases of non-binary and not pre-term these days!
			...
			child1.add(new Tree<String>(sentLat.getWord(start, end, newIndex)));  //YG REALLY?? cState??! not sure
					...
					for (int rp = 0; rp < scores[lp].length; rp++) {
						...
						if (matches(score, bestScore)) {
							...
							Tree<String> leftChildTree = extractBestViterbiParse(
									lState, lp, start, split, sentLat);
							Tree<String> rightChildTree = extractBestViterbiParse(
									rState, rp, split, end, sentLat);
							List<Tree<String>> children = new ArrayList<Tree<String>>();
							children.add(leftChildTree);
							children.add(rightChildTree);
	...
	// @YG this guy was removed in oldblatt, kept it here though it will
	// probably never be used
	public double computeTightThresholds(LatticeInput lat) {
		...
		for (int level = startLevel; level < endLevel; level++) {
			...
			initializeChartFromLattice(lat, curLexicon, level < 1, false, false);
			final boolean viterbi = true, logScores = true;
			if (level < 1) {
				doConstrainedViterbiInsideScores(curGrammar,
						level == startLevel);
	...
	private double getTightestThrehold(int start, int end, int state,
		...
		//boolean posLevel = (end - start == 1);
		boolean posLevel = !grammar.isGrammarTag(state);
		if (posLevel)
			return -2;
	...
	// @YG this guy was removed in oldblatt, kept it here though it will
	// probably never be used
	public void doGoldInsideOutsideScores(Tree<StateSet> tree, LatticeInput lat) {
		Grammar curGrammar = grammarCascade[endLevel - startLevel + 1];
		ExtLexicon curLexicon = (ExtLexicon)lexiconCascade[endLevel - startLevel + 1];
		...
		double initVal = 0;
		int level = isBaseline ? 1 : endLevel;
		createArrays(lat, false/* false */, curGrammar.numStates,
				curGrammar.numSubStates, level, initVal, false);

		// setGoldTreeCountsToOne(tree);
		initializeChartFromLattice(lat, curLexicon, false, true, false);
	...
	// @YG TODO this is entirely new! do I need to change anything here?
	...
	public CoarseToFineMaxRuleLatticeParser newInstance() {
		CoarseToFineMaxRuleLatticeParser newParser = new CoarseToFineMaxRuleLatticeParser(
