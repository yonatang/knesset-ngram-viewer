 ...
 * @author Slav Petrov
 */
public class GrammarTrainerPresplit {

   public static final String PRESPLIT_SEP=":";
	...
	public static class Options {
		...
		@Option(name = "-noStartSmooth", usage = "don't start with smoothing")
		public boolean noStartSmooth = false;

		@Option(name = "-lex2", usage = "use second lexicon")
		public boolean lex2 = true;

		// @Option(name = "-grsm", usage =
		// "Grammar smoothing parameter, in range [0,1].  (Default: 0.1)\n")
	...
	public static void main(String[] args) {
      ...
      boolean startWithSmoothing = !opts.noStartSmooth;
		...
		List<Tree<String>> validationTrees = Corpus.binarizeAndFilterTrees( corpus.getValidationTrees(), VERTICAL_MARKOVIZATION,
				HORIZONTAL_MARKOVIZATION, maxSentenceLength, binarization,
				manualAnnotation, VERBOSE);
		...
		if (VERBOSE) {
			for (int i = 0; i < numSubStatesArray.length; i++) {
				System.out.println("Tag " + (String) tagNumberer.object(i)
						+ " " + i + " substates:" + numSubStatesArray[i]);
			}
		}
		...
		StateSetTreeList trainStateSetTrees = createStateSetTreeList(trainTrees, numSubStatesArray, tagNumberer);
      if (validationTrees.size()>0) { throw new RuntimeException("never checked with validationTrees.size!=0"); }
		StateSetTreeList validationStateSetTrees = createStateSetTreeList(validationTrees, numSubStatesArray, tagNumberer);
		...
		if (opts.simpleLexicon) {
         throw new RuntimeException("simple lexicon is not supported");
		}
      /*
		if (opts.simpleLexicon) {
			System.out
      ...
      */
		...
		if (splitGrammarFile == null) {
			...
			Lexicon tmp_lexicon = opts.lex2 ?
             new SophisticatedLexicon2(numSubStatesArray, SophisticatedLexicon.DEFAULT_SMOOTHING_CUTOFF, smoothParams, new NoSmoothing(), filter)
         ...
         */
            ...
            throw new RuntimeException("featurized lexicon is not supported");
         }
         /*
			if (opts.featurizedLexicon)
         ...
         */
         ...
         //System.out.println("@@trained lexicon. scores:" + lexicon.score("HERMH",(short)tagNumberer.number("NN"),6,true,false)[2]);
			lexicon.tieRareWordStats(opts.rare);
         //System.out.println("@@tied lexicon. scores:" + lexicon.score("HERMH",(short)tagNumberer.number("NN"),6,true,false)[2]);
			lexicon.optimize();
         //System.out.println("@@optimized lexicon. scores:" + lexicon.score("HERMH",(short)tagNumberer.number("NN"),6,true,false)[2]);
      ...
      if (startWithSmoothing)
         startSplit=-1;
		for (int splitIndex = startSplit; splitIndex < numSplitTimes * 3; splitIndex++) {
         //System.out.println("@@at iter " + splitIndex + "  scores:" + maxLexicon.score("UNK-CAPS",(short)tagNumberer.number("NN"),6,true,true)[2]);

			// now do either a merge or a split and the end a smooth
			// on odd iterations merge, on even iterations split
			String opString = "";
			if (splitIndex==-1 || splitIndex % 3 == 2) {// (splitIndex==numSplitTimes*2){}
            ...
            /*
					lexicon = (opts.simpleLexicon) ? new SimpleLexicon(
							newNumSubStatesArray, -1, smoothParams, maxLexicon
									.getSmoother(), filter, trainStateSetTrees)
               ...
               */
            lexicon = opts.lex2 ?
							  new SophisticatedLexicon2( newNumSubStatesArray, SophisticatedLexicon.DEFAULT_SMOOTHING_CUTOFF, maxLexicon.getSmoothingParams(), maxLexicon .getSmoother(), maxLexicon .getPruningThreshold())
							: new SophisticatedLexicon( newNumSubStatesArray, SophisticatedLexicon.DEFAULT_SMOOTHING_CUTOFF, maxLexicon.getSmoothingParams(), maxLexicon .getSmoother(), maxLexicon .getPruningThreshold());

				boolean updateOnlyLexicon = true;
				double trainingLikelihood = GrammarTrainerPresplit.doOneEStep(grammar,
         ...
         //System.out.println("@@before before EM at iter " + splitIndex + "  scores:" + maxLexicon.score("UNK-CAPS",(short)tagNumberer.number("NN"),6,false,true)[2]);
			previousGrammar = grammar = maxGrammar;
			previousLexicon = lexicon = maxLexicon;
			droppingIter = 0;
         ...
         //System.out.println("@@before EM at iter " + splitIndex + "  scores:" + maxLexicon.score("UNK-CAPS",(short)tagNumberer.number("NN"),6,true,true)[2]);
			do {
				iter += 1;
               ...
               /*
					lexicon = (opts.simpleLexicon) ? new SimpleLexicon(
							grammar.numSubStates, -1, smoothParams, lexicon
									.getSmoother(), filter, trainStateSetTrees)
							: new SophisticatedLexicon(
                                 ...
                                 */
					lexicon = (opts.lex2) ? 
							  new SophisticatedLexicon2( grammar.numSubStates, SophisticatedLexicon.DEFAULT_SMOOTHING_CUTOFF, lexicon.getSmoothingParams(), lexicon .getSmoother(), lexicon .getPruningThreshold())
   ...
   /*
    * @@YG this is changed so that the number of substates for each tag is calculated from the trees.
    */
	public static short[] initializeSubStateArray(
      ...
      short nSubStateArray[] = calculateNumOfPresplitSubstates(trainTrees, tagNumberer);
   ...
   //@@YG new
   public static short[] calculateNumOfPresplitSubstates(List<Tree<String>> trees, Numberer tagNumberer) {
      Map<String,Short> tagToNumStates = new HashMap<String,Short>();
      for (Tree<String> tree: trees) {
         calculateNumOfPresplitSubstates(tree, tagNumberer, tagToNumStates);
      }

      short[] numSubStates = new short[tagNumberer.total()];
      Arrays.fill(numSubStates,(short)1);
      for (String tag : tagToNumStates.keySet()) {
         short numStates = (short) tagToNumStates.get(tag);
         short tagNumber = (short) tagNumberer.number(tag);
         numSubStates[tagNumber] = numStates;
      }
      return numSubStates;
   }
   //@@YG new
   private static void calculateNumOfPresplitSubstates(Tree<String> tree, Numberer tagNumberer, Map<String,Short> tagToNumStates) {
      if (tree.isLeaf()) { return; }
      String[] tag_split=tree.getLabel().split(PRESPLIT_SEP);
      String tag=null;
      if (tag_split.length==1) {
         tag = tag_split[0];
      }
      else if (tag_split.length>1) {
         tag = tag_split[0];
         Short prevNumSubstates = tagToNumStates.get(tag);
         if (prevNumSubstates==null || (prevNumSubstates < Short.valueOf(tag_split[1])+1) ) { tagToNumStates.put(tag, (short)(Short.valueOf(tag_split[1])+1)); }
      } else { assert(false); }
      tagNumberer.number(tag);
      for (Tree<String> child : tree.getChildren()) { calculateNumOfPresplitSubstates(child, tagNumberer, tagToNumStates); }
   }

   //@@YG new
   public static StateSetTreeList createStateSetTreeList(List<Tree<String>> trees, short[] numSubStatesArray, Numberer tagNumberer) {
      StateSetTreeList resTrees = new StateSetTreeList();
		for (Tree<String> tree : trees) {
			resTrees.add(presplitStringTreeToStatesetTree(tree, numSubStatesArray, tagNumberer));
			tree = null;
		}
      return resTrees;
	}
   ...
   //@@YG new
	public static Tree<StateSet> presplitStringTreeToStatesetTree(Tree<String> tree, short[] numStates, Numberer tagNumberer) {
		Tree<StateSet> result = presplitStringTreeToStatesetTree(tree, numStates, tagNumberer, false, 0, tree.getYield().size());
   ...
   //@@YG new
	private static Tree<StateSet> presplitStringTreeToStatesetTree(Tree<String> tree, short[] numStates, Numberer tagNumberer, boolean splitRoot, int from, int to) {
      final short zero = 0, one = 1;
		if (tree.isLeaf()) {
			StateSet newState = new StateSet(zero, one, tree.getLabel().intern(), (short) from, (short) to);
			return new Tree<StateSet>(newState);
		}
      String[] label_split = tree.getLabel().split(GrammarTrainerPresplit.PRESPLIT_SEP);
		short label = (short) tagNumberer.number(label_split[0]);
      short the_substate = label_split.length==1 ? 0 : Short.valueOf(label_split[1]);
		if (label < 0)
			label = 0;
		if (label >= numStates.length) {
			System.err.println("Have never seen this state before: "+tree.getLabel());
		}
		short nodeNumStates = numStates[label];
		if (!splitRoot)
			nodeNumStates = 1;
		StateSet newState = new StateSet(label, nodeNumStates, null, (short) from, (short) to);
      newState.initialSubstate = the_substate;
		...
		for (Tree<String> child : tree.getChildren()) {
			...
			Tree<StateSet> newChild = presplitStringTreeToStatesetTree(child, numStates, tagNumberer, true, from, from + length);
			from += length;
