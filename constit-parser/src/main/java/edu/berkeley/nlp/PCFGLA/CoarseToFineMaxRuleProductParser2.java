...
public class CoarseToFineMaxRuleProductParser2 extends CoarseToFineMaxRuleParser {
	...
	public CoarseToFineMaxRuleProductParser2(Grammar[] gr, Lexicon[] lex,
      ...
      //System.out.println("@@gr[1]:" + gr[1]+"\n\n@@@");
		...
		nLevels = (int) Math.ceil(Math.log(ArrayUtil.max(gr[1].numSubStates)) / Math.log(2));
		...
		if (initializeCascade) {
			...
			for (int nGr = 0; nGr < nGrammars; nGr++) {
               ...
               System.out.println(""+grammars[nGr].numStates + "!=" + numStates);
					System.exit(-1);
				}
	...
	public void initCascade(CoarseToFineMaxRuleProductParser2 otherParser) {
	...
	private void doCombinedMaxCScores(List<String> sentence, boolean scale) {
		...
		ArrayUtil.fill(maxcScore, Double.NEGATIVE_INFINITY);
		if (scale)
			System.err.println("Using scaling code");
						...
						for (int gr = 0; gr < nGrammars; gr++) {
							...
							if (rule == null) {
								System.err.println("Dont have rule "
										...
										+ " in grammar " + gr);
                        System.out.println(grammars[gr]);
                        System.exit(1);
								continue;
							}
							double[][] scores = rule.getScores2();
