 ...
 */
public class SophisticatedLexicon implements java.io.Serializable, ExtLexicon {
	...
	protected int unknownLevel = 5; // different modes for unknown words, 5 is
	// english specific
	/**
	...
	public Set<Short> getAllTags() {
		//System.out.println("Sophisticated Lexicon");
		return allTags;
	}
    ...
    * @return A double valued score, usually P(word|tag)
	...
	public double[] score(String word, short tag, int loc, boolean noSmoothing,
		...
		for (int substate = 0; substate < numSubStates[tag]; substate++) {
            ...
            //if (Double.isNaN(pb_W_T)) { //@@YG, for the unseen splits if pre-splitting
               //System.out.println("@@pb_W_T is NaN (known word)"); 
              // pb_W_T=0;
            //}
            ...
            //System.out.println("D@ unknown word:" + word + "\t with sig:" + sig);
				// iTW.word = sig;
               ...
               //if (substate==2) { //@@
               //   System.out.println("tag:"+tag+" c_TS:"+c_TS);
               //}
				}
				// if (c_TS == 0) continue;
				 ...
				 */
            if (priorProb>0) {
               pb_T_S = (0.01*pb_T_S) + (0.99*priorProb); //@YGYG this is my change for unknown words for now.
               //pb_T_S = (0.3*pb_T_S) + (0.7*priorProb); //@YGYG this is my change for unknown words for now.
               System.out.println("@@handling unknown word:" + word + " " + Numberer.getGlobalNumberer("tags").object(tag) + " " + pb_T_S + " " + priorProb );
            }
            if (substate == 2) { //"@@
            //   System.out.println("@@scores:" + word + "|" + sig + " " +pb_W_T + "," + pb_T_S + "," + p_W + "," + p_T+","+total+","+c_TS+","+c_S+"*"+unknownLevel+","+smooth[0]);
            }
            pb_W_T = pb_T_S * p_W / p_T;
            //System.out.println("word:" + word + "|" + sig + " :tag score:" + tag + " " + Numberer.getGlobalNumberer("tags").object(tag).toString() + "|" + substate + ": " + pb_W_T);
            //if (Double.isNaN(pb_W_T)) { //@@YG this is critical for smoothing to work and not mess everything up, (sometimes both pb_T_S and p_T are 0.. resulting in NaN)
             //  pb_W_T=0;
               //System.out.println("@@had NaN for unknown word:" + tag + " substate:" + substate + " sig:" + sig + " " + p_W + " " + p_T + " "  + pb_T_S);
            //}
			}
      ...
      boolean x=false;
		...
		for (int position = 0; position < words.size(); position++) {
			...
			for (short substate = 0; substate < nSubStates; substate++) {
               ...
               if (x && randomness==-1) {
                  System.out.println("@@rupdated signature:" + sig + " tag:" + tag + " substate:"+substate);
                  x=false;
               }
				}
				// if (secondHalf) {
