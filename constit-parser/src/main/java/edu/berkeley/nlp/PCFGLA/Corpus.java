...
public class Corpus {
	...
	private Corpus(String path, TreeBankType treebank, boolean onlyTest,
			int skipSection, boolean skipBilingual) {
		myTreebank = treebank;
		...
		if (dummy) {
			...
			case 8:
				sentences
						.add("((S (NP (PRP We)) (VP (VBP 're) (RB about) (VP (TO to) (VP (VB see) (SBAR (IN if) (S (NP (NN advertising)) (VP (VBZ works))))))) (. .)))");
				break;
			default:
	...
	private void loadChinese(String pathChinese, boolean skipBilingual)
		...
		if (!skipBilingual) {
			trainTrees.addAll(readTrees(pathChinese, 1, 25, Charset
					.forName("GB18030")));
			trainTrees.addAll(readTrees(pathChinese, 26, 270, Charset
					.forName("GB18030")));
		}
		trainTrees.addAll(readTrees(pathChinese, 400, 1151, Charset
				.forName("GB18030")));

		devTestTrees.addAll(readTrees(pathChinese, 301, 325, Charset
				.forName("GB18030")));
		validationTrees.addAll(readTrees(pathChinese, 301, 325, Charset
				.forName("GB18030")));
		finalTestTrees.addAll(readTrees(pathChinese, 271, 300, Charset
				.forName("GB18030")));
		System.out.print(""
	...
	private void loadWSJ(String pathWSJ, boolean onlyTest, int skipSection)
		...
		if (!onlyTest) {
			if (skipSection == -1)
				trainTrees.addAll(readTrees(pathWSJ, 200, 2199, Charset
						.defaultCharset())); // was 200, 2199
			else {
				System.out.println("Skipping section " + skipSection + ".");
				if (skipSection == 2) {
					trainTrees.addAll(readTrees(pathWSJ, 300, 2199, Charset
							.defaultCharset())); // was 200, 2199
				} else if (skipSection == 21) {
					trainTrees.addAll(readTrees(pathWSJ, 200, 2099, Charset
							.defaultCharset())); // was 200, 2199
				} else {
			...
			validationTrees.addAll(readTrees(pathWSJ, 2100, 2199, Charset
					.defaultCharset())); // was 2100, 2199
		}

		devTestTrees.addAll(readTrees(pathWSJ, 2200, 2299, Charset
				.defaultCharset()));
		finalTestTrees.addAll(readTrees(pathWSJ, 2300, 2399, Charset
				.defaultCharset()));
		System.out.println("done");
	}
	...
	public static List<Tree<String>> filterTreesForConditional(
		...
		List<Tree<String>> filteredTrees = new ArrayList<Tree<String>>(trees
				.size());
