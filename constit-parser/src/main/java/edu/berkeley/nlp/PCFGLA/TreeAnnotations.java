...
public class TreeAnnotations implements java.io.Serializable {
	...
	private static Tree<String> markGrammarNonterminals(Tree<String> tree, String parentLabel) {
		...
		if (tree.isPreTerminal()) {
         ...
         String[] lbl = tree.getLabel().split(GrammarTrainerPresplit.PRESPLIT_SEP);
         if (lbl.length==1) {
            verticallyMarkovizatedTree = new Tree<String>(lbl[0] + parentLabel, children);
         } else {
            assert(lbl.length==2);
            verticallyMarkovizatedTree = new Tree<String>(lbl[0] + parentLabel + GrammarTrainerPresplit.PRESPLIT_SEP + lbl[1], children);
         }
		}
		return verticallyMarkovizatedTree;
	}
	...
	static Tree<String> leftBinarizeTree(Tree<String> tree) {
		...
		if (tree.isLeaf())
			...
			return new Tree<String>(label, Collections
					.singletonList(leftBinarizeTree(children.get(0))));
		}
	...
	private static Tree<String> rightBinarizeTreeHelper(Tree<String> tree,
		...
		if (numChildrenLeft == 1) {
			...
			Tree<String> leftTree = rightBinarizeTreeHelper(tree,
					numChildrenLeft - 1, intermediateLabel + "_"
							+ rightTree.getLabel());
	...
	static Tree<String> headParentBinarizeTreeHelper(Binarization binarization,
		...
		if (leftChild == rightChild) {
			...
			kids.add(headParentBinarizeTree(binarization, children
					.get(leftChild)));
	...
	public static Tree<String> removeSuperfluousNodes(Tree<String> tree) {
		...
		for (String node : nodesInChain) {
			if (node.equals(parent) || node.equals(cLabel))
				continue;
			Tree<String> intermediate = new Tree<String>(node, result
					.getChildren());
