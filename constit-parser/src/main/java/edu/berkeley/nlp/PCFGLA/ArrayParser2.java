...
// @YG a modification of ArrayParser that allows ALL preterminal symbols in the constrained inside/outside calculation.
 ...
 */
public class ArrayParser2 implements Parser {
   protected Map<StateSet,StateSet[]> preterminalStates;
	...
	public ArrayParser2(Grammar gr, Lexicon lex) {
		this.touchedRules = 0;
		this.grammar = gr;
		this.lexicon = lex;
	...
	void doInsideScores(Tree<StateSet> tree, boolean noSmoothing,
      ...
      StateSet parent = tree.getLabel();
		...
		if (tree.isPreTerminal()) {
         ...
         StateSet[] parents = this.preterminalStates.get(parent);
         if (parents==null) {
            parents = new StateSet[this.grammar.numStates];
            for (short i=0;i<parents.length;i++) { 
               if (grammar.isGrammarTag(i)) continue;
               parents[i] = new StateSet(i,grammar.numSubStates[i]);
               parents[i].allocate();
            }
            this.preterminalStates.put(parent,parents);
         }
         for (StateSet _parent : parents) {
            if (_parent==null) { continue; }
            pState = _parent.getState();
            nParentStates = _parent.numSubStates();
            ...
            _parent.setIScores(lexiconScores);
            _parent.scaleIScores(0);
         }
		} else {
			...
			case 0:
				break;
			case 1:
				StateSet child_ = children.get(0).getLabel();
            StateSet[] childs = {child_};
            if (children.get(0).isPreTerminal()) { childs = this.preterminalStates.get(child_);}
            double[] iScores = new double[nParentStates];
            for (StateSet child : childs) {
               ...
               assert(child.getIScale()==1);
            }
            parent.setIScores(iScores);
            //parent.scaleIScores(child.getIScale());
            break;
			case 2:
				StateSet leftChild_ = children.get(0).getLabel();
            StateSet[] leftchilds = {leftChild_};
            if (children.get(0).isPreTerminal()) { leftchilds = this.preterminalStates.get(leftChild_);}

				StateSet rightChild_ = children.get(1).getLabel();
            StateSet[] rightchilds = {rightChild_};
            if (children.get(1).isPreTerminal()) { rightchilds = this.preterminalStates.get(rightChild_);}

				double[] iScores2 = new double[nParentStates];
				boolean foundOne2 = false;
            for (StateSet leftChild : leftchilds) {
               if (leftChild==null) { continue; }
               for (StateSet rightChild : rightchilds) {
                  ...
                  assert(leftChild.getIScale()==1);
                  assert(rightChild.getIScale()==1);
               }
            }
	...
	public double doInsideOutsideScores(Tree<StateSet> tree,
      ...
      this.preterminalStates = new HashMap<StateSet,StateSet[]>();
		...
		for (Iterator nodeI = t.subTreeList().iterator(); nodeI.hasNext();) {
			...
			for (int pos = 1; pos < path.size() - 1; pos++) {
				...
				Tree<String> intermediate = new Tree<String>(
						(String) tagNumberer.object(interState), parent
								.getChildren());
				List<Tree<String>> children = new ArrayList<Tree<String>>();
