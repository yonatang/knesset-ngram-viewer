...
public class SmoothAcrossParentBits implements Smoother, Serializable {
	...
	public SmoothAcrossParentBits(double smooth, Tree<Short>[] splitTrees) {
		...
		for (short state = 0; state < nStates; state++) {
            ...
            //@@YG is pre-splitting, should use < 2 because it can miss some branches
				//for (int branch = 0; branch < 2; branch++) {
				for (int branch = 0; branch < splitTree.getChildren().size(); branch++) {
	...
	public void smooth(short tag, double[] scores) {
		...
		for (int i = 0; i < scores.length; i++) {
            ...
            //if (Double.isNaN(scopy[i])) {
            //   System.out.println("@@NaN in smoothing:" + tag + " " + i + " " + scores[i] + " " + scores[k] + " " + diffWeights[tag][i][k]);
            //}
			}
		}
		for (int i = 0; i < scores.length; i++) {
