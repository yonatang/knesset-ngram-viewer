...
public class Grammar implements java.io.Serializable {
	...
	public void smooth(boolean noNormalize) {
		...
		for (UnaryRule unaryRule : unaryRuleCounter.keySet()) {
			...
			addUnary(unaryRule);
         //System.out.println("unary rule:" + unaryRule);
		}
		...
		for (BinaryRule binaryRule : binaryRuleCounter.keySet()) {
         ...
         //System.out.println("binary rule:" + binaryRule);
		}
		// Reset all counters:
	...
	public void tallyUninitializedStateSetTree(Tree<StateSet> tree) {
		...
		case 1:
         ...
         //@@YG  increment 0, just to initialize it
			unaryRuleCounter.incrementCount(urule, 0.0);
         //@@YG now increment the only the marked substates
			double[][] ucounts = new double[nChildSubStates][nParentSubStates];
			for (short i = 0; i < nChildSubStates; i++) {
				for (short j = 0; j < nParentSubStates; j++) {
               if (((child.initialSubstate < 0) || child.initialSubstate == i) &&  ((parent.initialSubstate < 0) ||parent.initialSubstate == j)) {
                  ucounts[i][j]=1.0;
               }
            }
         }
			unaryRuleCounter.incrementCount(urule, ucounts);
			break;
		case 2:
			StateSet leftChild = children.get(0).getLabel();
			short lChildState = leftChild.getState();
			...
			BinaryRule brule = new BinaryRule(parentState, lChildState,
					rChildState, bcounts);
         //@@YG  increment 0, just to initialize it
			binaryRuleCounter.incrementCount(brule, 0.0);
         //@@YG now increment the only the marked substates
         bcounts = new double[nLeftChildSubStates][nRightChildSubStates][nParentSubStates];
			for (short i = 0; i < nLeftChildSubStates; i++) {
				for (short j = 0; j < nRightChildSubStates; j++) {
               for (short k = 0; k < nParentSubStates; k++) {
                  if (     ((leftChild.initialSubstate < 0) || leftChild.initialSubstate == i) 
                        && ((rightChild.initialSubstate < 0) ||rightChild.initialSubstate == j)
                        && ((parent.initialSubstate < 0) ||parent.initialSubstate == k)) {
                     bcounts[i][j][k]=1.0;
                     //System.out.println("incrementing for rule:" + brule + "substates are:" + i + "," + j + "," + k);
                  }
               }
            }
         }
